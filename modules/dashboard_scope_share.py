...
"""
范围占比接口
接口：POST /api/dashboard/scope_share
入参：{"timeType":1}，1=日, 2=周, 3=月, 4=年
出参：从 summary.csv 返回单组范围占比数据，单位 %
"""
...
def build_payload(time_type: int) -> dict:
    ...
    p1 = float(row["scope1_share_avg"]) * 100.0
    p2 = float(row["scope2_share_avg"]) * 100.0
    p3 = float(row["scope3_share_avg"]) * 100.0
    ...
    return {
        ...
        "data": {
            ...
            "source": [
                {"name": "范围1", "data": p1, "dataWithUnit": f"{p1:.2f}{UNIT}"},
                {"name": "范围2", "data": p2, "dataWithUnit": f"{p2:.2f}{UNIT}"},
                {"name": "范围3", "data": p3, "dataWithUnit": f"{p3:.2f}{UNIT}"},
            ],
